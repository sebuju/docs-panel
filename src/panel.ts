...
const DEFAULT_STATE: PanelState = { split: 240, expanded: [], selected: null };
...
export class DocsPanel {
  ...
  private async status(relPath: string, value: unknown): Promise<void> {
    ...
    const ledger = await setStatus(this.root.uri, this.ledgerKey(relPath), status);
    this.postStatuses(ledger);
  }
  ...
  private async move(relPath: string, direction: "trash" | "restore"): Promise<void> {
    const base = this.base;
    if (!base || this.viewTrash !== (direction === "restore") || !this.root) {
      return;
    }
    ...
    const target =
      direction === "trash"
        ? vscode.Uri.joinPath(this.root.uri, TRASH_DIR, ...parts)
        : vscode.Uri.joinPath(this.root.uri, ...parts);
    ...
    try {
      ...
      await rekey(this.root.uri, this.ledgerKey(relPath), relativeTo(this.root.uri, to));
      this.state.selected = null;
